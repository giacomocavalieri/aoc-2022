(ns advent.2022.day-10
  (:require [advent.utils :refer [lines parse-int read-file unlines words]]
...
(defn cycles [ops]
  (let [starting-register-value 1]
    (reduce add-op-cycles [starting-register-value] ops)))

(defn part-a [ops]
  (let [cycles          (cycles ops)
        signal-strength (fn [cycle-number] (* cycle-number (get cycles (dec cycle-number))))]
